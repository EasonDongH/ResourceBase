                        ...
                        <ul>
                            <c:forEach items="${articleList}" var="a">
                                <li style="padding: 5px">
                                    <a href="/article/${a.id}" title="ajax实现form表单提交" target="_blank">${a.articleTitle}</a>
                                </li>
                            </c:forEach>
                        </ul>
                    </div>
                            ...
                            <c:forEach items="${categoryList}" var="c">
                                <c:if test="${c.pid==0}">
                                    <li class="cat-item" style="padding: 5px">
                                        <a href="/category/${c.id}">丨- ${c.categoryName}</a>
                                    </li>
                                    <ul class="children">
                                        <c:forEach items="${categoryList}" var="c2">
                                            <c:if test="${c2.pid==c.id}">
                                                <li class="cat-item" style="padding: 5px 30px;">
                                                    <a href="/category/${c2.id}" target="_blank">${c2.categoryName}</a>
                                                </li>
                                            </c:if>
                                        </c:forEach>
                                    </ul>
                ...
                <div class="layui-colla-item">
                    ...
                    <div class="layui-colla-content layui-show">
                        <c:forEach items="${tagList}" var="t">
                            <a href="/tag/${t.id}" target="_blank">${t.tagName}</a>
                        </c:forEach>
                    </div>
                </div>
            </div>
